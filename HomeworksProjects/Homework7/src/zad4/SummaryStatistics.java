...
public class SummaryStatistics {
    ...
    // метод, който пресмята и отпечатва средната стойност от печалбата (revenue) на списъка от
    // компании companies , генериран с метода setup() на class SummaryStatistics
    public double companyStatsWithStreamAverage() {
        double averageRevenue = companies.stream()
                .mapToDouble(Company::getRevenue)// mapToDouble because it is a stream specialized to work with primitives
                .average()
                .orElse(0.0);

        System.out.println("Company stats with stream average: " + averageRevenue);
